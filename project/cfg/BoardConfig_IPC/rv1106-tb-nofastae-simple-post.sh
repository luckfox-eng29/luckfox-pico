...
echo "================================================================================"
if find $RK_PROJECT_PACKAGE_ROOTFS_DIR -type f -name $RK_AIISP_MODEL;then
	echo "Found config RK_AIISP_MODEL: $RK_AIISP_MODEL"
	rm -fv `find $RK_PROJECT_PACKAGE_ROOTFS_DIR -name "*.aiisp" | grep -v $RK_AIISP_MODEL`
fi
if [ "$RK_AIISP_MODEL" = "NONE" ];then
	rm -fv `find $RK_PROJECT_PACKAGE_ROOTFS_DIR -name "*.aiisp" `
	rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/librkpostisp.so
fi
echo "================================================================================"

echo "================================================================================"
if [ -n "$RK_NPU_MODEL" ];then
if find $RK_PROJECT_PACKAGE_ROOTFS_DIR -type f -name $RK_NPU_MODEL;then
	rm `find $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/ -name "*.data" | grep -v $RK_NPU_MODEL`

	if [ ! "${RK_NPU_MODEL}x" = "object_detection_pfp.datax" ];then
		mv $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/$RK_NPU_MODEL \
			$RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/object_detection_pfp.data
	fi
fi
fi
echo "================================================================================"
